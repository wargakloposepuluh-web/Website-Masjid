...
export default function KegiatanMingguanPage() {
  ...
  const fetchKegiatan = async () => {
    setLoading(true);
    try {
      const res = await fetch("/api/kegiatan/rutin?tipe=MINGGUAN");
      if (res.ok) {
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-[1450px] mx-auto space-y-6 text-[#111317]">
      ...
      <div className="no-print flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-white/50 backdrop-blur-md p-6 rounded-3xl border border-white/70 shadow-[0_4px_20px_rgba(0,0,0,0.03)]">
        <div>
          <h1 className="text-xl sm:text-2xl font-bold tracking-tight text-slate-900 flex items-center gap-2.5">
            ...
            <span>Jadwal Kegiatan Mingguan</span>
          </h1>
          <p className="text-xs sm:text-sm text-slate-500 mt-1">
            Kegiatan ibadah, kajian keislaman, dan pembacaan sholawat yang berlangsung rutin setiap pekan.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2.5 flex-wrap">
          ...
          >
            ...
            <span>Tambah Kegiatan Mingguan</span>
          </button>
        </div>
      </div>
      ...
      <div className="hidden print:block mb-6 text-center border-b-2 border-black pb-4">
        ...
        <h3 className="text-base font-bold mt-2 underline">
          JADWAL KEGIATAN RUTIN MINGGUAN MASJID
        </h3>
      ...
      {/* Grid Cards Kegiatan */}
      {loading ? (
        <div className="p-12 text-center text-slate-400 text-sm">Memuat kegiatan mingguan...</div>
      ) : items.length === 0 ? (
        <div className="p-12 text-center text-slate-400 text-sm bg-white/50 rounded-3xl border border-white/70">
          Belum ada kegiatan mingguan yang dicatat.
        </div>
      ) : (
            ...
            >
              <div className="space-y-3">
                <div className="flex items-start justify-between gap-2">
                  <span className="text-[10px] font-bold px-2 py-0.5 rounded-lg bg-teal-50 text-teal-700 border border-teal-200/60 uppercase">
                    Mingguan
                  </span>
                  <span
                ...
                {item.pengisi && (
                  <div className="pt-2 border-t border-slate-100 text-xs">
                    <span className="text-slate-400 block text-[10px]">Pengisi / Penceramah:</span>
              ...
              <div className="no-print pt-3 border-t border-slate-100 flex items-center justify-between">
                <span className="text-[11px] text-slate-400">
                  PJ: {item.penanggungJawab || "Takmir"}
                </span>
